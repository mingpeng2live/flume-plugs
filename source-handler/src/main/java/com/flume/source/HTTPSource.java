...
public class HTTPSource extends AbstractSource implements EventDrivenSource, Configurable {
   ...
   */
    ...
    public void configure(Context context) {
        try {
            ...
            if (sslEnabled) {
                ...
                Preconditions.checkArgument(keyStorePath != null && !keyStorePath.isEmpty(), "Keystore is required for SSL Conifguration");
    ...
    private class FlumeHTTPServlet extends HttpServlet {
        ...
        public void doPost(HttpServletRequest request, HttpServletResponse response)
            ...
            try {
                ...
                if (CollectionUtils.isEmpty(events)) {
                    LOG.info("Received event is empty. " + events.size());
                    return;
                }
            ...
            try {
                /** 当存在批量事件的时候使用 */
                if (StringUtils.isNotEmpty(request.getHeader(Constant.ISBATCH))) {
        ...
        public void setCookieId(HttpServletRequest request, HttpServletResponse response) {
            /** 设置cookie */
            Map<String, Cookie> cookieMap = Utils.readCookieMap(request);
            Cookie cookie = cookieMap.get(Constant.UID);
            if (cookie == null) {
                int maxAge = 0;
    ...
    private static class HTTPSourceSocketConnector extends SslSocketConnector {
        ...
        /**
         * Instantiates a new Http source socket connector.
         *
         * @param excludedProtocols the excluded protocols
         */
